# Progress Tracker

## Active lesson

- Lesson 2: Tools and Structured Inputs — [[01-lessons/02-tools/lesson]]

## Completed

- Academy scaffold created
- Obsidian vault setup + graph view enabled
- `.venv` created at repo root with Python 3.13 (`langchain-ollama`, `langchain-core`, `rich` installed)
- [[02-projects/project-01-goal-planner/README|Project 1]] — fully built, persistent REPL agent:
  - **Stage 1 — Brainstorm agent**: iterative Q&A loop, spinner + dim thinking panels, loops until `READY_TO_PLAN`
  - **Stage 2 — Planner agent**: streams a structured plan using full Q&A context
  - **Stage 3 — Follow-up chat**: session stays alive, ask anything about the plan
  - Slash commands: `/save` `/model` `/new` `/clear` `/help` `/exit`
  - Model picker at startup — any locally available Ollama model
- Week 1 reflection written — [[04-memory-vault/reflections/week-01]]
- Lesson 1 complete — [[01-lessons/01-foundations/lesson]]

## Next milestones

1. Read [[01-lessons/02-tools/lesson|Lesson 2: Tools]]
2. Complete [[01-lessons/02-tools/exercises|Lesson 2 exercises]]
3. Build [[02-projects/project-02-tool-agent/README|Project 2]]: first tool-using agent
4. Write concept notes: [[04-memory-vault/concepts/tools-vs-prompts]] and [[04-memory-vault/concepts/react-pattern]]

## See also

- [[00-dashboard/roadmap]]
- [[00-dashboard/next-steps]]
